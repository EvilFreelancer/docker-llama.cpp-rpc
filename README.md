...
которая расположена в директории `./models` в корне данного проекта. Директория `./models` в свою очередь монтируется
внутрь контейнера `main` и доступна по пути `/app/models`.
