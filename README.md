...
Сборка под 32-битный arm/v7 не выполняется, llama.cpp эту архитектуру больше не поддерживает. Бинарники в образах переносимые. На amd64 в образ входят варианты CPU-бэкенда под разные поколения процессоров, нужный выбирается при запуске. В CUDA-образе ядра собраны под Ampere и новее (RTX 30xx/40xx, A100, Hopper, compute capability 8.0+). Для более старых карт соберите образ сами, указав архитектуру через `--build-arg CUDA_ARCHITECTURES=61-real` (пример для Pascal).
...
| APP_MEM            | 1024                                           | Количество Мб оперативной памяти доступной клиенту, в режиме CUDA это количество оперативной памяти видеокарты. В сборках из llama.cpp b6790 и новее игнорируется, rpc-server определяет свободную память сам | 
